...
public class McpServerConfigurationLoader implements EnvironmentPostProcessor {
	...
	public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
		log.info("开始处理 MCP 服务器配置...");
		String enabled = environment.getProperty("spring.ai.mcp.enabled", "true");
		log.info("MCP 配置状态: enabled={}", enabled);

		if (!Boolean.parseBoolean(enabled)) {
			log.info("MCP 配置已禁用，跳过处理");
			return;
		}
		...
		try {
			...
			log.info("加载配置文件: {}", resource.getURL());
			...
			log.info("创建临时配置文件: {}", tempConfigPath);

			// 读取并处理配置
			try (InputStream is = resource.getInputStream()) {
				JsonNode rootNode = objectMapper.readTree(is);
				log.debug("原始配置内容: {}", objectMapper.writeValueAsString(rootNode));
				...
				if (rootNode.has("mcpServers")) {
					...
					mcpServers.fields().forEachRemaining(entry -> {
						...
						if (serverConfig.has("command") && "npx".equals(serverConfig.get("command").asText())) {
							...
							log.info("更新服务器 {} 的命令: {} -> {}", serverName, "npx", npxCommand);
						}
					});

					// 将处理后的配置写入临时文件
					objectMapper.writerWithDefaultPrettyPrinter().writeValue(tempConfigPath.toFile(), rootNode);
					log.debug("处理后的配置内容: {}", objectMapper.writeValueAsString(rootNode));
					...
					log.info("已更新配置文件路径: {}", newConfigPath);

					log.info("MCP 服务器配置处理完成，当前操作系统: {}", getOsType());
				}
				else {
